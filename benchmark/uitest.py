...
class JustAUITest(UITestCase):
    ...
    def test_copy(self):
        return
        with self.ui_test.load_file(get_url_for_data_file('writer_doc_1.docx')):
            for i in range(5):
                self.xUITest.executeCommand('.uno:GoToEndOfNextPageSel')
            self.xUITest.executeCommand('.uno:Copy')
            with self.ui_test.load_file(get_url_for_data_file('writer_doc_2.docx')):
                for i in range(5):
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with self.ui_test.load_file(get_url_for_data_file('writer_doc_1.docx')):
                ...
                with self.ui_test.load_file(get_url_for_data_file('writer_doc_2.docx')):
                    ...
                    # test save_0
                    list_save_time = []
                    save_path = os.path.join(tempdir, "writer_doc_2_saveas.docx")
                    save_start_time = 0.0
                    with self.ui_test.execute_dialog_through_command(".uno:SaveAs", close_button="open") as x_save_dialog:
                        ...
                        select_by_text(x_file_type, "Word 2010–365 Document (.docx)")
